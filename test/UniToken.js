...
contract('UniToken', function(accounts){
	...
	it('initializes the contract with the correct values', function(){
		return UniToken.deployed().then(function(instance){
			...
			assert.equal(name, 'Uni Token', 'has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol) {
			assert.equal(symbol, 'UNV','has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard) {
			assert.equal(standard,'UNV Token v1.0','makes sure it has the correct standard');
		});
	});
	...
	it('transfers token ownership', function(){
		return UniToken.deployed().then(function(instance){
			tokenInstance = instance;
			//Test 'require' statement by transfering something larger than the sender's balance
			...
			assert.equal(receipt.logs[0].args._from, accounts[0],'logs the ammount the tokens are transfered from');
			assert.equal(receipt.logs[0].args._to, accounts[1], 'logs the ammount the tokens are transfered to');
			assert.equal(receipt.logs[0].args._value, 250000, 'logs the ammount of tokens being transfered');
			return tokenInstance.balanceOf(accounts[1]);
		}).then(function(balance){
			assert.equal(balance.toNumber(),250000, 'adds the ammoubt to the receiving account')
			return tokenInstance.balanceOf(accounts[0]);
		}).then(function(balance){
	...
	it('handles delegated token transfers', function(){
		return UniToken.deployed().then(function(instance){
			...
			//Transfer something higher than the from Balance
			return tokenInstance.transferFrom(fromAccount,toAccount, 9999,{from: spendingAccount });
		}).then(assert.fail).catch(function(error){
			assert(error.message, 'cannot transfer value higher than balance');
			//Transfer something higher than the allowed balance
			return tokenInstance.transferFrom(fromAccount,toAccount, 20,{from: spendingAccount });
		}).then(assert.fail).catch(function(error){
			assert(error.message, 'cannot transfer value higher than approved ammount');
			...
			assert.equal(receipt.logs[0].args._value, 10, 'logs the ammount of tokens being transfered');
